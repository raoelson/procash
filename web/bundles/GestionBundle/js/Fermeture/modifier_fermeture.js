...
//} else {
//    dateNow.setDate($('#procash_gestionbundle_saisifermeture_dateDebutFermeture').val());
//}
...
if (delaiMax !== 'undefined') {
    ...
    dateMaxDelai.setDate(dateMaxDelai.getDate() + 1);
}
...
if (delaiMax !== 'undefined' && seuil !== 'undefined') {
    ...
    myCalendarDeb.setInsensitiveRange(null, $('#procash_gestionbundle_saisifermeture_dateDebutFermeture').val());
    myCalendarFin.setInsensitiveRange(null, $('#procash_gestionbundle_saisifermeture_dateDebutFermeture').val());
}
myCalendarDeb.attachEvent("onClick", function () {
    if ($("#procash_gestionbundle_saisifermeture_dateDebutFermeture").val() != '') {
        ...
        if (delaiMax !== 'undefined') {
            ...
            myCalendarDeb.setSensitiveRange($('#procash_gestionbundle_saisifermeture_dateDebutFermeture').val(), null);
        }
...
if ($("#procash_gestionbundle_saisifermeture_dateDebutFermeture").val() != '') {
    var newDate1 = $('#procash_gestionbundle_saisifermeture_dateDebutFermeture').val();
    myCalendarFin.setSensitiveRange(newDate1, formatDateSlash(dateMaxDelai));
}
...
var fnValidationFermetureModif = function () {
    var modificationFerm = true;
    var valide = 0;
    ...
    if (dd.trim() != '' && df.trim() != '') {
        if (modificationFerm) {
            $(listeFermetures).each(function (k, v) {
                if (ddParDefaut == v.dd && dfParDefaut == v.df && v.user == clientPardefaut) {
                    listeFermetures.splice(k, 1);
                }
            });
        }
        var ddParam = dd;
        ...
        if (valide == 0) {
            $('#formModifFermeture').submit();
            $('#erreurChevauchement').remove();
        } else {
            var msg = '<div class ="alert alert-danger col-md-6 erreurChevauchementModif" id="erreurChevauchementModif" style="text-align:center!important">Cet utilisateur a une fermeture inclus dans cette période.</div>';
            $('div#footerFermetureModif').prepend(msg);
            $('.validerModif').attr('disabled', true);
            $("#erreurChevauchementModif").delay(4000).fadeOut(function () {
                $('#erreurChevauchementModif').remove();
                $('.validerModif').attr('disabled', false);
            });
        }
    } else {
